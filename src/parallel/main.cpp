...
int main(int argc, char* argv[]) {
	...
	if (numprocs > 1) {
		...
		if (myid == 0) {
			...
			fprintf(stdout, "Running in parallel with %d processors\n", numprocs);
			...
			for (time_step = start_time; time_step < max_time_step + 1; time_step++) {
				...
				// central task receives partially complete buffer from each worker, including species and seed positions as well as extant species array  at sites assigned to that worker
	...
	// single processor version
		...
		fprintf(stdout, "Running in series with %d processor\n", numprocs);
